...
if st.button('Hae Data'):
    ...
    if publication_data and publication_data['data']:
        ...
        if selected_field_of_study != 'All':
            ...
            relevant_lens_ids = fields_of_study_df.loc[fields_of_study_df['field_of_study'] == selected_field_of_study, 'lens_id'].tolist()

            
            if relevant_lens_ids:
                st.write(f"Debug: Relevant lens_ids {relevant_lens_ids}")  
                ...
                if not filtered_publications_df.empty:
                    ...
                    st.error("No publications found for the selected field of study. Please check the field selection.")
            else:
                
                st.error("No matching lens_ids found for the selected field of study. Please verify the data and selections.")
        else:
